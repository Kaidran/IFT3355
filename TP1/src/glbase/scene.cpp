...
bool Node::Intersect(vec3 world_pos)
{
	...
	// BEGIN CODE HER
	auto boundingBox = this->GetFullBoundingBox();
	vec3 vmin = boundingBox.min;
	vec3 vmax = boundingBox.max;
	if ((world_pos.x >= vmin.x && world_pos.x <= vmax.x) &&
		(world_pos.y >= vmin.y && world_pos.y <= vmax.y) &&
		(world_pos.z >= vmin.z && world_pos.z <= vmax.z))
	{
		intersect = true;
	}
...
// TODO :: Box for enemies is stuck far back
void Node::ComputeBoundingBox()
{
	// Compute the bounding box of the current node (don't forget transformations ...)
	// BEGIN CODE HERE

	// apply full transform the boundaries
	auto transform = this->fullTransform();
	vec3 tmp[8] = this->boundaries;
	vec3 min = tmp[0];
	vec3 max = tmp[0];
	// having problems converting v3[array] to v4[array]
	// so i build it manually to apply the transform
	vec4 v0(tmp[0], 1);
	vec4 v1(tmp[1], 1);
	vec4 v2(tmp[2], 1);
	vec4 v3(tmp[3], 1);
	...
	v6 = transform*v6;
	v7 = transform*v7;
	vec4 v_all[8] = {v0,v1,v2,v3,v4,v5,v6,v7};


	for (uint i=0; i < 8; i++)
	{
		min.x = glm::min(max.x, v_all[i].x);
		min.y = glm::min(max.y, v_all[i].y);
		min.z = glm::min(max.z, v_all[i].z);

		max.x = glm::max(min.x, v_all[i].x);
		max.y = glm::max(min.y, v_all[i].y);
		max.z = glm::max(min.z, v_all[i].z);
	}
...
Cylinder::Cylinder(uint iterations, vec4 color, double height)
{
	...
	// TODO :: compute vertex on both bases depending on angle
	//         do triangles between top bottom. alternate.
	decimal angle = 0.0;
	decimal delta = 2*pi()/(3*iterations);
	// vertex in the center of the the base to triangularize circle
	while (angle <= 2*pi()) 
	{
		decimal x = cos(angle)/2;
		decimal y = sin(angle)/2;
	...
	uint base1 = vertices.size()-2;
	uint base2 = vertices.size()-1;

	for (uint i = 0; i < vertices.size()-4; i+=2)
	{
		indices.push_back(ivec3(base1,i,i+2));
		indices.push_back(ivec3(base2,i+1,i+3));
		indices.push_back(ivec3(i, i + 1, i + 2));
		indices.push_back(ivec3(i+2, i+1,i+3));
	}
	// close the last iteration with start of solid
	indices.push_back(ivec3(base1,base1-2,0));
	indices.push_back(ivec3(base2,base2-2,1));
	indices.push_back(ivec3(base2-2,0,1));
	indices.push_back(ivec3(base1-2,0,1));
...
Pyramid::Pyramid(vec4 color)
{
	...
	VertexPositionNormal vertices[18] = {
		//sides
		{ vec3(0, 0, 0), vec3(0.5, 1.0, 0.5) },
		{ vec3(0.5, 1.0, 0.5), vec3(0, 0, 1) },
		{ vec3(0, 0, 1), vec3(0, 0, 0) },

		{ vec3(0, 0, 1), vec3(0.5, 1.0, 0.5) },
		{ vec3(0.5, 1.0, 0.5), vec3(1, 0, 1) },
		{ vec3(1, 0, 1), vec3(0, 0, 1) },

		{ vec3(1, 0, 0), vec3(0.5, 1.0, 0.5) },
		{ vec3(0.5, 1.0, 0.5), vec3(1, 0.5, 1) },
		{ vec3(1, 0, 1), vec3(1, 0, 0) },
		
		{ vec3(1, 0, 0), vec3(0.5, 1.0, 0.5) },
		{ vec3(0.5, 1.0, 0.5), vec3(0, 0, 0) },
		{ vec3(0, 0, 0), vec3(1, 0, 0) },

		//base
		{ vec3(0, 0, 0), vec3(0, 0, 1) },
		{ vec3(0, 0, 1), vec3(1, 0, 1) },
		{ vec3(1, 0, 1), vec3(0, 0, 0) },
