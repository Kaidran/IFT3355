...
void SPH::moveParticles( float deltaTime )
{
    ...
    for ( int i=0 ; i<_particles.size() ; ++i )
    {
        ...
        // BEGIN TODO //
        Particle& particle = _particles[i];
        QVector3D speed = particle.velocity() + deltaTime * particle.acceleration();

        particle.setVelocity(speed);
        QVector3D position = particle.position() + deltaTime * particle.velocity();

        Ray particle_ray(particle.position(), speed);
        Intersection intersect;
        _container.intersect(particle_ray, intersect);
        float time = deltaTime;
        //float epsilon = 1e-8;
        while (_container.intersect(particle_ray, intersect) && intersect.rayParameterT() <= time)
        {
            // displacement
            auto e = (1e-2)*intersect.normal().normalized();
            auto plan_norm = intersect.normal();
            QVector3D intersect_e = intersect.position()-e;
            QVector3D projection_v = QVector3D::dotProduct(speed, plan_norm)*plan_norm.normalized();

            position = intersect_e;
            speed = speed - projection_v;
            time -= intersect.rayParameterT();

            particle_ray = Ray(position, speed);
    ...
    for ( int i=0 ; i<_particles.size() ; ++i )
    {
        ...
        if (_grid.cellIndex(position) != particle.cellIndex())
        {
            _grid.addParticle(_grid.cellIndex(position),i);
            _grid.removeParticle(particle.cellIndex(),i);
            particle.setCellIndex(_grid.cellIndex(position));
        }
        // ... //
    ...
    for ( int j=0 ; j<neighborhood.size() ; ++j )
    {
        ...
        for ( int k=0 ; k<neighbors.size() ; ++k )
        {
            ...
            if ( r2 < _smoothingRadius2 )
            {
                ...
                float kernelMass = densityKernel(r2) * neighbor.mass();
                density += kernelMass;
                normal += difference*densitykernelGradient(r2)*neighbor.mass();
            }
        }
    }
